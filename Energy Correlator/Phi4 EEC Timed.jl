...
open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "w") do io
    println(io, "Elapsed Time - N=$N, a=$a, l=$l, m=$mass, r1=$r1, r2=$r2, t1i=$t1i, t1f=$t1f, t2i=$t2i, t2f=$t2f, dt=$dt")
end
for (i, t1) in enumerate(t1_range)
    psiL, time, etc... = @timed tdvp(H, -im*t1, psi; nsteps=round(Int, t1/time_step), maxdim=maxdim, cutoff=cutoff, normalize=false)
    open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
        println(io, "t1=$t1 - Apply exp(-iHt1): $time s")
        println(io, ". . . - B. dims: $(linkdims(psiL))")
    end

    for (j, t2) in enumerate(t2_range)
        if t1 != t2
            psiR, time, etc... = @timed tdvp(H, -im*t2, psi; nsteps=round(Int, t2/time_step), maxdim=maxdim, cutoff=cutoff, normalize=false)
            open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
                println(io, "t2=$t2 - Apply exp(-iHt2): $time s")
                println(io, ". . . - B. dims: $(linkdims(psiR))")
            end
        else
            psiR = copy(psiL)
        end
        ...
        if r2 > 0
            psiR, time, etc... = @timed apply(T0i(r2, dir=1), psiR)
        else
            psiR, time, etc... = @timed apply(T0i(r2, dir=-1), psiR)
        end
        open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
            println(io, ". . . - Apply T0x(r2): $time s")
            println(io, ". . . - B. dims: $(linkdims(psiR))")
        end

        if t1 != t2
            psiR, time, etc... = @timed tdvp(H, im*(t2-t1), psiR; nsteps=round(Int, abs(t2-t1)/time_step), maxdim=maxdim, cutoff=cutoff, normalize=false)
            open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
                println(io, ". . . - Apply exp(iH(t2-t1)): $time s")
                println(io, ". . . - B. dims: $(linkdims(psiR))")
            end
        end

        if r1 > 0
            ME, time, etc... = @timed inner(psiL', T0i(r1, dir=1), psiR) / norm_psi
        else
            ME, time, etc... = @timed inner(psiL', T0i(r1, dir=-1), psiR) / norm_psi
        end
        open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
            println(io, "t1=$t1, t2=$t2 - Calc. mat. el.: $time s")
            println(io, "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -")
        end
    ...
    open("Time Logs/EEC_Timed,N=$N,a=$a,l=$l,m=$mass,r1=$r1,r2=$r2,t1i=$t1i,t1f=$t1f,t2i=$t2i,t2f=$t2f,dt=$dt.txt", "a") do io
        println(io, "x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x")
    end
end
